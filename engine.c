...
#define encode_move(from, to, piece, promotion, capture,  double, en_passant, castling) \
    ((from) | ((to) << 6) | ((piece) << 12) | ((promotion) << 16) | ((capture) << 20) | ((double) << 21) | ((en_passant) << 22) | ((castling) << 23))
...
static inline int get_lsb_index(U64 bitboard) {
    if (bitboard) {
        count_bits((bitboard & -bitboard) - 1);
    }
    else {return -1;}
}
...
static inline U64 get_rook_attacks(int square, U64 block){
...
static inline U64 get_queen_attacks(int square, U64 block){
    U64 result = 0ULL;
...
static inline int apply_move(int move, int only_capture_flag) {
...
int parse_move(char *move_string){
    ...
    for (int move_count = 0; move_count < move_list->move_count; move_count++){
        ...
        if (source_square == decode_move_source(move) && target_square == decode_move_target(move)){
            ...
            if ((~ promoted_piece) | (promoted_pieces[promoted_piece] ==  move_string[4])){
                return move;
            }
        }
    }
